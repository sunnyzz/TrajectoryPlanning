...
void TrajectoryPlan::LinearInterpolation()
{
	...
	while (Request)
	{
		...
		{
			Velocity_new_X = (Timer/1000.0)*ActualAccelerration_x+velocity_x;
			Position_new_X = 0.5*ActualAccelerration_x*(Timer/1000.0)*(Timer/1000.0)+velocity_x*(Timer/1000.0)+position_x;
			...
			{
				Velocity_new_Y = Acceleration_time_y*ActualAccelerration_y+velocity_y;
				Position_new_Y = 0.5*ActualAccelerration_y*Acceleration_time_y*Acceleration_time_y+(Velocity_new_Y-velocity_y) * ((Timer / 1000.0) - Acceleration_time_y)+velocity_y*(Timer/1000)+position_y;
			...
			{
				Velocity_new_Y = (Timer/1000.0)*ActualAccelerration_y+velocity_y;
				Position_new_Y = 0.5*ActualAccelerration_y*(Timer/1000.0)*(Timer/1000.0)+position_y+velocity_y*(Timer/1000.0);
				std::cout<<"when t= "<<Timer<<" ms,Velocity_Y= "<<Velocity_new_Y<<" mm/s,Position_Y= "<<Position_new_Y<<" mm\n";
				std::cout<<"next cycle: \n";
			}
		...
		{
			Velocity_new_X = Acceleration_time_x*ActualAccelerration_x+velocity_x;
			Position_new_X = 0.5*ActualAccelerration_x*Acceleration_time_x*Acceleration_time_x+(Velocity_new_X-velocity_x)*((Timer / 1000.0) - Acceleration_time_x)+velocity_x*(Timer/1000.0)+position_x;
			std::cout << "when t= " << Timer << " ms,Velocity_X= " << Velocity_new_X << " mm/s,Position_X= " << Position_new_X << " mm\n";
			//再输出Y
            if (((Acceleration_time_y + Uniform_motion_time_y - (Timer / 1000.0))>=0) && ((Acceleration_time_y - (Timer / 1000.0)) <= 0.001))
			{
				Velocity_new_Y = Acceleration_time_y*ActualAccelerration_y+velocity_y;
				Position_new_Y = 0.5*ActualAccelerration_y*Acceleration_time_y*Acceleration_time_y + (Velocity_new_Y - velocity_y)*((Timer / 1000.0) - Acceleration_time_y) + velocity_y*(Timer / 1000.0) + position_y;
					;
				std::cout << "when t= " << Timer << " ms,Velocity_Y= " << Velocity_new_Y << " mm/s,Position_Y= " << Position_new_Y << " mm\n";
				std::cout << "next cycle: \n";Request = false;
